...
![Edgex Foundry架构图](https://docs.edgexfoundry.org/2.0/general/EdgeX_architecture.png "Edgex Foundry架构图")
...
此外，设备元数据和设备的创建也可以通过访问EdgeX网关的前端页面`http://{edgex-ip}:{edgex-port}`手动实现。

![Edgex Foundry前端页面](front.png "Edgex Foundry前端页面")

`设备脚本文件`需要通过docker容器部署

目前课题三系统的远程接入接口（edgex-ip、core-metadata-service-port、edgex-port、mqtt-broker-port）暂未发布至公网，如有需要，可按需发布公网访问接口。